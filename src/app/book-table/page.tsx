...
import { Employees, TableType } from "@/types/types";

const BookTable = () => {
	const { user } = useUser();
	// user_2fmwuOJt3tWTZTfkTl0XdCzmQNd
	...
	// 			const sortedData = tablesData.sort((a, b) =>
	// 				a.title.localeCompare(b.title)
	// 			);
	// 			setTables(sortedData);
	// 		} else {
	// 			console.error("Data fetched is not an array:", tablesData);
	// 			toast.error("Received data is not valid.");
	// 		}
	// 	} catch (error) {
	...
	const fetchEmployees = async () => {
		try {
			const response = await fetch(
				`${apiUrl}/api/employees?externalId=${user?.id}`,
				{
					method: "GET",
					headers: {
